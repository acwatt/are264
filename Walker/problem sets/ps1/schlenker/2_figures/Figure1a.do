...
use fips year month time*
	...
	using ../dataSTATA/weather_corn, clear;
...
estimates use ../resultSTATA/panelRegression/corn_chebyTemperature-3_36_year1950_2020_month3_8_clusterState;
...
estimates use ../resultSTATA/panelRegression/corn_piecewiseLinear10_29_36_year1950_2020_month3_8_clusterState;
gen bLin = 0 if (temperature <= 10);
qui replace  bLin = _b[ddayMod]*(temperature-10) if (temperature > 10) & (temperature <= 29);
qui replace  bLin = _b[ddayMod]*19 + _b[ddayHot]*(temperature-29) if (temperature > 29) & (temperature <= 36);
qui replace  bLin = _b[ddayMod]*19 + _b[ddayHot]*7 if (temperature > 36);

*-------------------------------------------------------------------------------------------;
* load regression results - dummy variables for 3C intervals;
estimates use ../resultSTATA/panelRegression/corn_dummyTemperature-3_36_year1950_2020_month3_8_clusterState;
gen tempDummy = .;
gen bDummy = .;
local i = 1;
forvalues b = 0(3)33 {;
...
twoway 	(rarea lb ub temperature, color(gs8%30) yaxis(1))
	   ...
	   ytitle("Log Corn Yields (Bushel/Acre)", axis(1) color(black)) ylabel(-0.06(0.02)0.02, axis(1)) yscale(range(-0.08 0.02) axis(1))
